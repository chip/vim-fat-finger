...
function! FatUnset()
	let counter = 0
	let command = "iunabbrev"
	while counter < len(s:fat)
		try
			exec command . " " . s:fat[counter][0] 
		catch /No such/
		endtry
		let counter += 1
	endwhile
...
function! FatSet()
	...
	while counter < len(s:fat)
		try
			exec command . " " . s:fat[counter][0] . " " . s:fat[counter][1] 
		catch /.*/
...
command -nargs=0 FatSet call FatSet()
command -nargs=0 FatUnset call FatUnset()
